export class JoinValidator{
    ...
    onLoadDocument() {
        $_b("#join-form-send-btn").addEventListener("click", this.onClickSendButton.bind(this));
    }
    ...
    checkValidate() {
        ...
        if(!passwordRegex.test($("#password").val().trim()) || !passwordRegex.test($("#repassword").val().trim())) {
        ...
        if(!birthRegex.test($("#birth").val().trim())) {
    ...
    onSuccessJoin(result) {
        $_b("#join-dialog").classList.toggle("visible");
        setTimeout(function () {
            alert("메인화면으로 옮기기");
        }, 2);
    }
