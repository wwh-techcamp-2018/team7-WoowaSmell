...
@Repository
public interface RestaurantRepository extends JpaRepository<Food, Long> {
    ...
    @Query(value = "select avg(star_point) as avg_point from (select id as food_id, name, o_id from (select id, restaurant_id, name from food where food.restaurant_id = ?1) f join  (select id as o_id, food_id from order_food) o on o.food_id = f.id) a join review on review.order_food_id = a.o_id", nativeQuery = true)
    List<Map<String, String>> getRestaurantStarPoint(Long restaurantId);
